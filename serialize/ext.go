...
var extFuncMaps = map[reflect.Type]ext.Encoder{reflect.TypeOf(exttime.Encoder): exttime.Encoder}
var extFuncs = []ext.Encoder{exttime.Encoder}
...
func SetExtFunc(f ext.Encoder) {
	t := reflect.TypeOf(f)
	// ignore time
	if t == reflect.TypeOf(exttime.Encoder) {
		return
	}
